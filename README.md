...
> REST api 설계규칙을 지키며 개발하였습니다.  
> 팀 프로젝트 이후 개인적으로 연장하여 코딩한 내용입니다.
> 구현된 내용은 다음과 같습니다.  
...
* 마이 페이지
  * 회원 정보를 확인할 수 있음
  * 회원 정보는 Spring boot에서 thymeleaf에서 지원해주는 frament를 이용해 로그인한 user정보를 담고 이를 불러오는 식으로 함
  * 회원가입 시에는 주소 입력하는 부분이 없지만, 마이페이지에서는 주소를 입력하여 새로 저장할 수 있음
  * 회원가입중 필수 정보는 DB에서 mst테이블에 저장, 수정 정보는 dtl에 따로 나누어서 저장함
  * 회원탈퇴 가능  
...
1개월의 기간을 가지고 홈페이지를 분석하며 설계하고 어떤식으로 할 지 정한 후 클론코딩을 진행했습니다.
REST API 설계 규칙에 따라 주소를 설정하였고, DB설계와 Javascript를 이용한 동적인 구현을 연습하기 위해 프로젝트를 진행했습니다.
아래는 프로젝트를 하며 아쉬웠거나 배웠던 점입니다.

1. 처음 설계에 대해 신중하게 해야 한다는 것을 깨달았습니다.

	설계에 대해서 가장 크게 깨달았던 것이 상품이었습니다. 클론코딩을한 웍스아웃의 경우 상품마다 각각 사이즈가 있으며, 
	상품마다 사이즈의 갯수도 다 다르기 때문에 데이터베이스에서 테이블을 나눌 때 하나하나 다 나눠야 하는 것을 프로젝트 중간에
	깨달았습니다.

	뜯어고치기에는 시간이 지체될 것 같아서 그룹별 코드로 묶어서 수습했습니다만 좀 더 쉽게 풀 수 있었던 것을 초반 설계때문에 장바구니를
	할때도, 상품 구매를 할 때도 초반에 했던 문제 때문에 코드도 복잡해지고 생각할 것도 많아지게 되었습니다.


2. 생각한 것 보다 더 디테일에 신경을 많이 써야된다는 점입니다.

	처음에는 홈페이지를 따라하느라 바빴지만 어느정도 감을 잡고 여유가 생기다 보니 드는 생각이 내가 이 홈페이지를 이용하면 어떤 점이 불편한가 였습니다.
	내가 이 홈페이지를 이용할 때 이런식으로 만들면 불편하지 않을까, 이 부분이 있으면 좀 더 편리하지 않을까 하는 점이었습니다.
	이런식으로 개발하다 보니 모든것을 고려한다는 점에서 시간이 오래 걸렸지만, 디테일하게 해야한다는 점을 깨달았습니다.


3. 시간 부족에 시달렸다는 점입니다.

	위와 같이 디테일에 신경을 쏟다보니 홈페이지의 모든 기능들을 다 구현하지 못했던 점이 아쉬웠습니다.
	물론 1개월의 시간 이후에 개인적으로 1개월 반정도를 투자하여 프로젝트를 어느정도 완성시켰지만, 약속했던 1개월동안 
	다 못했다는 점에 다시 한번 부족함을 느꼈지만, 이로 인해 다음 프로젝트에서는 좀 더 성장할 수 있는 모습을 보여줄 수 있을거라 생각합니다.

 
## 프로젝트 소감

첫 프로젝트를 진행하면서 어려운 점도 많았지만, 프로젝트를 하기 전과 후는 확실히 다르다는 것을 깨달았습니다.
어려운 점도 많았고 잘 안풀릴 때도 있었지만 어떻게든 풀어나가려 노력했고, 이로 인해 이전보다 더 성장할 수 있는 계기가 되었다고 생각합니다.
프로젝트를 하면서 직접 Spring Framework를 이용해 DB설계, Rest API 규약, MVC패턴이용, AOP, Security등 많은 것을 배울수 있어서 좋았습니다.
2개월 반 만들면서 어느 정도 완성된 프로젝트를 만들었다고 생각하며, 이 후에는 설계와 코드에 신경써서 좀 더 깔끔한 프로젝트를 만들 수 있도록 개선해보겠습니다.
